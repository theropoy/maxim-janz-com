...
import { FaWhatsapp as WhatsappIcon, FaInstagram as InstagramIcon, FaEnvelope as MailIcon, FaGithub as GithubIcon } from 'react-icons/fa';
...
const Contact: React.FC<contactProps> = ({reff, ger}) => {
        ...
        <section className={styles.container} ref={reff}>
            ...
            <div className={styles.content}>
                ...
                <div className={styles.right}>
                        {
                            contactList.map((item, key) => makeInfo(item, key))
                        }
                </div>
            </div>
            <form action="" className={styles.form} onSubmit={formHandler}>
                <input type="text" className={styles.inputName}  placeholder={ger? "Ihr Name" : "Your name"} onChange={(e) => setName(e.target.value) } value={name}/>
                <input type="text" className={styles.inputMail}  placeholder={ger? "Ihre E-Mail" : "Your email"} onChange={(e) => setMail(e.target.value) } value={mail}/>
                <textarea name="" className={styles.inputText}cols={30} rows={10} placeholder={ger? "Ihre Nachricht" : "Your message"} onChange={(e) => setMsg(e.target.value) } value={msg} ></textarea>
                <button type="submit" 
                    className={`${styles.formButton} ${!buttonActive? styles.formButtonDisabled : ""} `}
                >{buttonActive ? (ger? "Versenden" : "Send") : (ger? "Bitte alles Ausfüllen" : "Please fill out everything")}</button>
            </form>
        </section>
    )
}
